...
export default function ProduccionReporteServilleta({ usuario }) {
  ...
  const [idsTipoBobinaServilleta, setIdsTipoBobinaServilleta] = useState([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        const dataTipos = await ObtenerTiposBobinaServilleta();
        setTipos(dataTipos);
      } catch (e) {
  ...
  const cargarCatalogo = async () => {
    ...
    setError("");
    try {
      const data = await verProduccionesServilleta({
        FechaInicio: aFechaISO(rangoFechas?.from),
        FechaFin: aFechaISO(rangoFechas?.to),
        IdTurno: idTurno || null,
        IdsTipoBobinaServilleta: idsTipoBobinaServilleta.length
          ? idsTipoBobinaServilleta
          : null,
  ...
  const alternarTipo = (idTipoBobinaServilleta) => {
    setIdsTipoBobinaServilleta((prev) =>
      prev.includes(idTipoBobinaServilleta)
        ? prev.filter((id) => id !== idTipoBobinaServilleta)
        : [...prev, idTipoBobinaServilleta],
    );
    setPagina(1);
  };
  ...
  const hayFiltros =
    rangoFechas?.from ||
    idTurno ||
    idsTipoBobinaServilleta.length > 0 ||
    codigoBobina ||
    operador ||
    idEstadoProduccion;
  ...
  const descargarInforme = async () => {
    ...
    try {
      await descargarReporteProduccionServilletaPorPeriodo(
  ...
  const verDetalle = async (produccion) => {
    ...
    try {
      if (produccion.NombreEstadoProduccion === "Cancelada") {
        await descargarReporteProduccionServilletaCancelada(
          produccion.IdProduccionServilleta,
        );
      } else {
        await descargarReporteDetalleProduccionServilleta(
          produccion.IdProduccionServilleta,
          true,
        );
      }
          ...
          <section className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            ...
            <div className="flex flex-col gap-5 p-5">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <DoubleDatePicker
                  id="rango-produccion-servilleta"
              ...
              {tipos.length > 0 && (
                <div className="flex flex-col gap-1.5">
                  ...
                  <ToggleGroup
                    value={idsTipoBobinaServilleta.map(String)}
                    className="flex flex-wrap justify-start gap-2"
                  >
                    {tipos.map((t) => (
                      <ToggleGroupItem
                        key={t.IdTipoBobinaServilleta}
                        value={String(t.IdTipoBobinaServilleta)}
                        onClick={() => alternarTipo(t.IdTipoBobinaServilleta)}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {catalogo.Producciones.map((p) => (
                        <TableRow key={p.IdProduccionServilleta}>
                          <TableCell>
                            <Badge
                              variant="outline"
                              className={cn(
                          ...
                          <TableCell>{p.NombreTipoBobinaServilleta}</TableCell>
                          <TableCell className="font-mono text-[12.5px]">
                            {p.CodigoBobina}
                          </TableCell>
                          <TableCell>{p.DescripcionMedida}</TableCell>
                              ...
                              <TooltipTrigger
                                render={
                                  <Button
                                    variant="ghost"
                                    size="icon"
                                    disabled={descargandoId === p.IdProduccionServilleta}
                                    onClick={() => verDetalle(p)}
                                    className="h-9 w-9 text-slate-400 hover:bg-c4/10 hover:text-c3"
                                  >
                                    {descargandoId === p.IdProduccionServilleta ? (
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-[12.5px] text-slate-500">
                            {p.NombreTipoBobinaServilleta} · {p.DescripcionMedida} ·{" "}
                            {p.NombreTurno}
                          </span>
                        </div>
                        <Badge
