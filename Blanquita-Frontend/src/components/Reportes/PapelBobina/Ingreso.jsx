...
export default function IngresoReporteBobinaPapel() {
  ...
  const cargarCatalogo = async () => {
    ...
    setError("");
    try {
      const data = await verLotesBobinaPapel({
  ...
  }, [rangoFechas, idProveedor, idsTipoBobina, pagina]);
  ...
  const hayFiltros = rangoFechas?.from || idProveedor || idsTipoBobina.length > 0;
  const rangoCompleto = Boolean(rangoFechas?.from && rangoFechas?.to);

  const descargarInforme = async () => {
    setDescargandoInforme(true);
    try {
      await descargarReporteLotesPorPeriodo(
  ...
  const verDetalle = async (idLoteBobina) => {
    setDescargandoId(idLoteBobina);
    try {
      await descargarReporteLoteDetalle(idLoteBobina);
    } catch (e) {
      toast.error(e.message);
    } finally {
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Tooltip>
              ...
              <TooltipContent>
                {rangoCompleto
                  ? "PDF con una tabla por lote del rango elegido (fecha, proveedor y bobinas) más el total general"
                          ...
                          <TooltipTrigger
                            render={
                              <Button
                                variant="ghost"
                                size="icon"
                                disabled={descargandoId === l.IdLoteBobina}
                                onClick={() => verDetalle(l.IdLoteBobina)}
                                className="h-9 w-9 text-slate-400 hover:bg-c4/10 hover:text-c3"
                              >
                                {descargandoId === l.IdLoteBobina ? (
